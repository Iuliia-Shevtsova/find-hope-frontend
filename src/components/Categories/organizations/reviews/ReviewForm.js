...
const ReviewForm = ({organization_name, review, handleSubmit, handleChange, handleUdate, setRating, error, isEdit}) =>{

    const rating = [5,4,3,2,1].map((score, index) => {
        return (
          <Fragment key={index}>
            <input type="radio" value={score} checked={review.score == score} onChange={()=>console.log('onChange')} name="rating" id={`rating-${score}`}/>
